...
class Particle:
    ...
    def move(self, step_len):
        ...
        if random() < social_distancing_probability and particle_list.x_vals[-1] >= social_distancing_start_time:
            for other in particle_list:
                if other is not self:
                    dx, dy = self.position[0] - other.position[0], self.position[1] - other.position[1]
                    if 0 < dx*dx + dy*dy < (social_distancing_radius * 2)**2:
                        self.velocity_dir = math.atan2(dy, dx)
                        break
