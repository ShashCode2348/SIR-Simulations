...
class ParticleList(list):
    ...
    def update_graph(self):
        ...
        #R_value = round((self.I_vals[-1] - self.I_vals[-2] - self.S_and_I_vals[-1] + self.S_and_I_vals[-2])/(self.I_vals[-2] * recovery_rate), 2) if self.I_and_R_vals[-2] > self.R_vals[-2] else 0
...
axes.fill_between(particle_list.x_vals, particle_list.I_and_R_vals, population, color="#4C88F7", alpha=1, label='Susceptible')
