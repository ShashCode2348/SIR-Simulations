...
class ParticleList(list):
    def __init__(self, particles=[]):
        ...
        self.I_vals, self.S_and_I_vals = [0], [population]
        self.x_vals = [0.0]
    ...
    def step(self, trans_rate, recovery_rate, infection_radius, step_len):  #Modify to prevent escaping from box (see shell in editor for how)
        # move all particles
        ...
        for s in susceptibles:       
            for i in infected:
                if (s.position[0] - i.position[0])**2 + (s.position[1] - i.position[1])**2 <= infection_radius**2:
                    if random() < trans_rate:
                        s.state = 1
                        break
    ...
    def update_graph(self):
        self.I_vals.append(sum(1 for p in self if p.state == 1))
        self.S_and_I_vals.append(sum(1 for p in self if p.state != 2))
        ...
        axes.fill_between(self.x_vals, 0, self.I_vals, color="#ED5151", alpha=1, label='Infected')
        axes.fill_between(self.x_vals, self.I_vals, self.S_and_I_vals, color="#4C88F7", alpha=1, label='Susceptible')
        axes.fill_between(self.x_vals, self.S_and_I_vals, population, color="#676767", alpha=1, label='Removed')
        pygame.draw.rect(screen, (255, 255, 255), (W_offset, H_offset, W_sim, H_sim), 2)
        screen.blit(fig, (0, -75))  #type: ignore
        R_value = round((self.I_vals[-1] - self.I_vals[-2] - self.S_and_I_vals[-1] + self.S_and_I_vals[-2])/(self.I_vals[-2] * recovery_rate), 2) if self.I_vals[-2] > 0 else 0
    ...
    def main_loop(self):
        ...
        while running:
            ...
            self.step(trans_rate, recovery_rate, infection_radius, step_len)
...
#Matplotlib setup
graph_width, graph_height = 750, 800
...
I_fill = axes.fill_between(particle_list.x_vals, 0, particle_list.I_vals, color="#ED5151", alpha=1, label='Infected')
S_fill = axes.fill_between(particle_list.x_vals, particle_list.I_vals, particle_list.S_and_I_vals, color="#4C88F7", alpha=1, label='Susceptible')
R_fill = axes.fill_between(particle_list.x_vals, particle_list.S_and_I_vals, population, color="#676767", alpha=1, label='Removed')
